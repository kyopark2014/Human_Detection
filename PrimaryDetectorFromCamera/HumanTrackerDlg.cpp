...
void CHumanTrackerDlg::HumanDetection(int num,CString ImageFile) {
	...
	for(j=0;j<iHeight/RATE;j++) {        
		for(i=0;i<iWidth/RATE;i++) {     
			for(m=0;m<RATE;m++) {
				for(n=0;n<RATE;n++) {     
					motion[i*RATE+m][j*RATE+n] = mt->motion_object[i][j];
				}
			}
		}
	}
...
//		SaveTargetInfo(num,t); 
	...
	for(t=0;t<MTG;t++) {            
		if(m_candidate[t].valid != 0) {
			...
			if(hasmotionorcandidate==false)   mt->VerifyCandidate(t,MINMOTIONRATE);
			else							mt->VerifyCandidate(t,MINUPDATERATE);	
		}            
	}
...
//	SaveDetectionResult(num);
	...
	lt->DoLocalTracker(m_candidate,target,motion,img_current);     
...
//	FILE *fp;
//	CString fname;
//	fname.Format("targetid%d.txt",num);
//	fp = fopen(fname,"w");

	for(t=0;t<MTG;t++) {
		if(lt->objectid_current[t].lock==true) {
			...
			mt->DrawBox(pixels,2,hx0,hy0,hx1,hy1);

//			fprintf(fp,"%d (valid) (%d %d %d %d) (%d %d %d %d)\n",t,bx0,by0,bx1,by1,hx0,hy0,hx1,hy1);
		}			
	}
//	fclose(fp);
...
void CHumanTrackerDlg::ShowImages()
{
	...
	// save id number in the image
	for(int t=0;t<MTG;t++) {
		if(lt->objectid_current[t].lock==true) {
			int bx0 = lt->objectid_current[t].bx0;
			int by1 = lt->objectid_current[t].by1;

			CString str_id;
			str_id.Format("%d",lt->objectid_current[t].id);
			dcMem.TextOut(bx0,iHeight-by1-16,str_id);
		}			
	}	

...
CMotionTracker::CMotionTracker(int rate_s)
{
	rate = rate_s;
}
...
//	MaxMotionWeight = 0;
//	int MinMotionWeight = 15;

	for(int i=0;i<iWidth;i++) {
		for(int j=0;j<iHeight;j++) {
			motion_weight[i][j] = CheckMotionWeight(i,j);
			if(motion_weight[i][j]>MinMotionWeight)   motion_object[i][j]=1;
			else                                      motion_object[i][j]=0;
		}
	} 
}
